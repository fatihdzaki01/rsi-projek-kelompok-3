...
class SearchController extends Controller
{
    ...
    public function communities(Request $request)
    {
        ...
        $isFollowingSubquery = $user 
            ? 'EXISTS(SELECT 1 FROM follow_komunitas WHERE follow_komunitas.id_komunitas = komunitas.id_komunitas AND follow_komunitas.id_user = ' . (int)$user->id_user . ' AND follow_komunitas.is_active = true)'
            : 'false';

        $query = DB::table('komunitas')
            ->leftJoin('wilayah', 'komunitas.kode_wilayah', '=', 'wilayah.kode')
            ->select([
                'komunitas.id_komunitas as id',
                'komunitas.id_komunitas',
                'komunitas.nama_lembaga as nama_komunitas',
                'komunitas.nama_lembaga',
                'komunitas.deskripsi',
                'komunitas.foto_lembaga_url as foto_komunitas',
                'komunitas.foto_lembaga_url',
                'komunitas.nomor_kontak',
                'komunitas.link_medsos',
                'komunitas.status',
                'wilayah.nama as kota',
                'wilayah.nama as nama_wilayah',
                DB::raw('(SELECT COUNT(*) FROM follow_komunitas WHERE follow_komunitas.id_komunitas = komunitas.id_komunitas AND follow_komunitas.is_active = true) as total_follower'),
                DB::raw('(SELECT COUNT(*) FROM campaign WHERE campaign.id_komunitas = komunitas.id_komunitas AND campaign.status = \'aktif\') as total_campaign_aktif'),
                DB::raw('COALESCE((SELECT SUM(dana_terkumpul) FROM campaign WHERE campaign.id_komunitas = komunitas.id_komunitas), 0) as total_dana_diterima'),
                DB::raw($isFollowingSubquery . ' as is_following')
            ])
