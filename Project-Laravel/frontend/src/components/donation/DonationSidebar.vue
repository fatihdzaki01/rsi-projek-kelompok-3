<template>
  ...
  >
    ...
    <div class="flex flex-col gap-2">
      ...
      <button
        @click="handleGoToMonitoring"
        class="w-full py-2.5 rounded-lg bg-card border border-[#2C2C2C] text-[#2C2C2C] text-sm font-semibold hover:bg-secondary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
        Lihat Monitoring
      </button>
...
function handleGoToMonitoring() {
  if (auth.user?.role === 'KOMUNITAS' || auth.user?.role === 'SUPERADMIN') {
    router.push(`/dashboard/campaigns/${props.campaignId}/internal`)
  } else {
    router.push(`/monitoring/${props.campaignId}`)
  }
}
