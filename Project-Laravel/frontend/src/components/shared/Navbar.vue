...
async function fetchKomunitasId() {
  if (isLoggedIn.value && user.value?.role === 'KOMUNITAS') {
    try {
      const res = await api.get('/communities/profile')
      idKomunitas.value = res.data.data.id_komunitas
    } catch (err) {
      console.error('Gagal mengambil profil komunitas:', err)
    }
  } else {
    idKomunitas.value = null
  }
}
...
const navLinks = computed(() => {
  ...
  if (isLoggedIn.value && user.value?.role === 'KOMUNITAS') {
    links.push({ label: 'Dashboard', path: '/community/dashboard' })
    if (idKomunitas.value) {
      links.push({ label: 'Campaign', path: `/communities/${idKomunitas.value}` })
    } else {
      links.push({ label: 'Campaign', path: '/community/dashboard' })
    }
    links.push({ label: 'Grafik', path: '/communities/grafik-donasi' })
  } else {
    links.push({ label: 'Campaign', path: '/campaigns' })
    links.push({ label: 'Komunitas', path: '/communities' })
  }
  return links
})
...
watch(() => route.path, () => {
  loadUser()
  if (isLoggedIn.value && user.value?.role === 'KOMUNITAS' && !idKomunitas.value) {
    fetchKomunitasId()
  }
  updateIndicator()
})
  ...
  <header class="sticky top-0 z-50 h-16 w-full border-b border-gray-100 bg-white shadow-sm">
    <div class="mx-auto flex h-full max-w-7xl items-center justify-between px-4 lg:px-6">
      ...
      <nav ref="navRef" class="relative hidden items-center md:flex">
        ...
        <router-link
          v-if="isLoggedIn && user?.role === 'KOMUNITAS'"
          to="/communities/pencairan"
          :class="[
            'relative px-4 py-5 text-sm transition-colors',
            route.path.startsWith('/communities/pencairan')
              ? 'nav-link-active font-medium text-[#8B4513]'
              : 'text-gray-700 hover:text-[#8B4513]',
          ]"
        >Pencairan</router-link>
      </nav>
      ...
      <div class="hidden items-center gap-3 md:flex">
        ...
        <template v-if="isLoggedIn">
          <router-link v-if="user?.role !== 'KOMUNITAS'" to="/donations/history" class="text-sm text-gray-700 hover:text-[#8B4513] transition-colors" :class="route.path.startsWith('/donations') ? 'font-medium text-[#8B4513]' : ''">Donasi Saya</router-link>
    ...
    <div v-if="isMobileMenuOpen" class="border-b border-gray-100 bg-white px-4 pb-4 md:hidden">
      ...
      <nav class="flex flex-col gap-1">
        ...
        <button
          v-if="user?.role === 'KOMUNITAS'"
          @click="() => { router.push('/communities/pencairan'); isMobileMenuOpen = false }"
